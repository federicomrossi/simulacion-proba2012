...
Se puede ver que las probabilidades de que salga cada número, no son exactamente las esperadas, pero se acercan a este valor. Cuanto mayor el número de ejecuciones, las probabilidades se van acercando más al valor calculado teóricamente. 
...
\subsection{Análisis del algoritmo}
	...
	Si se asume que \textbf{\textit{L}} es una variable aleatoria, entonces el método para calcular el valor de longitud esperado, o la longitud media, se extrae del cálculo de la esperanza para una variable aleatoria discreta. Esto es:\\
	 
	 $\mathbb{E}(\textbf{\textit{L}}) = \sum\limits_{i=1}^n l_{i} P_{i}(l_{i}) $ \\
 con \textit{n} la cantidad de secuencias, siendo \textit{$l_i$} la longitud de la secuencia \textit{i} y \textit{$P(l_i)$} la probabilidad de \textit{$l_i$}. \\
 Entonces se puede simplificar la fórmula a:\\
$\mathbb{E}(\textbf{\textit{L}}) = {1 \over n} \sum\limits_{i=1}^n l_{i} $,\\
que es lo mismo que se calcula en el algoritmo anterior, o sea, la suma de las longitudes dividido la cantidad de secuencias.
